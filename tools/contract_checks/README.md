...
That import is dropped from the contract entirely.
...
- It is `eval()`'d in a namespace containing only the imported class.
...
- **PR lane** (`check-externals:run`): runs on every PR touching Python, non-blocking during the stabilization period. Status check name: `check-externals`.
- **Nightly lane** (`check-externals:run --rebuild-cache`): runs at 03:00 UTC against freshly-resolved constraints. On failure, auto-opens a GitHub issue tagged `check-externals,bug` (mirrored to Discord via the existing `.github/workflows/discord-issues.yml` bridge).
- **Framework self-tests** (`check-externals:test`): runs on PRs that touch `tools/contract_checks/**`.
