...
from contract_checks.engine_env import installed_version, read_constraints
...
def verify_heavy_class(hc: HeavyClass) -> CheckResult:
    ...
    The construction expression is ``eval()``'d in a fresh namespace
    containing only the imported class symbol. The eval is wrapped in a
    2-second wall-clock cap (``signal.alarm`` on POSIX, watcher thread on
    Windows) — a timeout means the manifest entry is wrong: either the SDK
    is doing I/O during ``__init__``, or the construct expression is doing
    something heavier than dummy-arg construction.
...
def verify_constraint_match(package: str) -> CheckResult:
    ...
    Args:
        package: Top-level package name (case-insensitive — constraints
                 file is normalised to lowercase by
                 :func:`engine_env.read_constraints`).
    ...
    """
    pinned = read_constraints().get(package.lower())
    if pinned is None:
        return CheckResult(
            ok=True,
