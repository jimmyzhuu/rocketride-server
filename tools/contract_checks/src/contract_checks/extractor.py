...
class _FileVisitor(ast.NodeVisitor):
    ...
    def _extract_alternative_imports(self, try_node: ast.Try) -> list[ImportRequirement]:
        """
        Collect import statements from a try-body + its except-body alternatives.
        ...
        Returns:
            ...
            seen in the try body or any handler body. Side effect: also
            updates ``self._aliases`` so any chains using these imports
            still resolve.
        """
...
def _apply_manifest(contract: ComponentContract) -> None:
    ...
    * **skip_packages:** drops auto-extracted AND manifest entries for the
    ...
    if manifest.skip_packages:
        ...
        contract.any_of_imports = [
            g
            for g in contract.any_of_imports
            if not all(alt.module.split('.', 1)[0] in manifest.skip_packages for alt in g.alternatives)
        ]
